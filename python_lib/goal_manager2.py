...
class MissionPlanningNode(Node):
    ...
    def __init__(self, goals_yaml: Path, loop_num: int) -> None:
        ...
        self.subscription = self.create_subscription(
            ...
            self.goal_callback,
            10,
        )  # https://autowarefoundation.github.io/autoware-documentation/pr-278/design/autoware-interfaces/ad-api/types/autoware_adapi_v1_msgs/msg/route_state/
    ...
    def timer_callback(self) -> None:
        ...
        if self.curr_route_state is None:
            return
        self.get_logger().info(f"{self.curr_route_state=}")
        if self.curr_route_state == 0:  # UNKNOWN
            pass
        elif self.curr_route_state == 1:  # UNSET
            self.publish_goal()
        elif self.curr_route_state == 2:  # SET
            self.call_engage_service()
        elif self.curr_route_state == 3:  # ARRIVED
            self.publish_goal()

    def goal_callback(self, msg: RouteState) -> None:
        """Handle the RouteState message."""
        self.curr_route_state = msg.state
    ...
    def arrived_callback(self) -> None:
        """Handle the arrived state."""
